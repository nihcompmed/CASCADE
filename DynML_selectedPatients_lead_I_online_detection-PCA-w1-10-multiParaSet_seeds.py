...
def compute_topological_entropy(trajectory, eps_values):
    """
    trajectory shape: (T, 3*N) — full joint trajectory
    Split into N individual 3D trajectories and compute entropy per oscillator.
    """
    ...
    for eps in eps_values:
        per_oscillator_entropies = []
        for k in range(N):
            # Extract the k-th oscillator's 3D trajectory
            traj_k = trajectory[:, 3*k : 3*k + 3]   # shape (T, 3)
            unique_orbits = set()
            for point in traj_k:
                rounded = tuple(np.round(point / eps))
                unique_orbits.add(rounded)
            N_n_eps = len(unique_orbits)
            h_k = np.log(N_n_eps) / T
            per_oscillator_entropies.append(h_k)

        entropy_estimates[eps] = np.mean(per_oscillator_entropies)
...
save_dir = f"results_all_selected_patients_multiPara_seeds_newtopocalculation"
