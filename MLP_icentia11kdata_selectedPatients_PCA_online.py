...
save_dir = f"results_all_selected_patients_MLP"
os.makedirs(save_dir, exist_ok=True)

results_csv = os.path.join(
    save_dir,
    f"results_summary_MLP_online_detection_PCA_all_patient_{record}_data_seed_{data_seed}_res_seed_{res_seed}_input_len_{input_len}_batch_size_{batch_size}_hidden_dim_{hidden_dim}.csv"
)
...
for pred_len in prediction_lengths:
    ...
    if (str(record), int(data_seed), int(reservoir_seed), int(input_len), int(pred_len), int(batch_size), int(hidden_dim)) in existing_configs:
        print(f"Skipping existing config: Patient={record}, Data_seed={data_seed}, Reservoir_seed = {reservoir_seed}, input_len={input_len}, Pred_len={pred_len}, batch_size={batch_size}, hidden_dim={hidden_dim}")
        continue
    ...
    if torch.cuda.is_available():
        torch.cuda.manual_seed(reservoir_seed)
        torch.cuda.manual_seed_all(reservoir_seed)


    # ====== Initialize Model ======
    model = MLPRegressor(pca_comp, hidden_dim, pred_len).to(device)
    optimizer = optim.Adam(model.parameters(), lr=lr)
    criterion = nn.MSELoss()  
    
    # ====== Prepare data for PyTorch ======
    # X_y_all_pca: (n_samples, pca_comp)
    # Y_all_raw.T:    (n_samples, pred_len)
    X_t = torch.from_numpy(X_y_all_pca).float().to(device)   # shape (n_samples, pca_comp)
    Y_t = torch.from_numpy(Y_all_raw.T).float().to(device)   # shape (n_samples, pred_len)
    ...
    for seg_idx, seg in enumerate(train_segments):
        ...
        while start + pred_len <= len(seg):
            ...
            # --- prepare window as one sample with input_len features ---
            y_in = seg[start - input_len:start].reshape(1, -1)          # shape: (1, input_len)
            
            # --- PCA transform the input window (1 sample -> pca_comp features) ---
            y_in_pca = pca_train.transform(y_in)                        # shape: (1, pca_comp)
            
            # Predict using MLP
            with torch.no_grad():
                x_t = torch.from_numpy(y_in_pca).float().to(device)     # shape: (1, pca_comp)
                y_pred = model(x_t).cpu().numpy().reshape(-1)           # shape: (pred_len,)
    ...
    print("Prediction done for all segments.")
    ...
    print("Validation prediction done for all segments.")
    ...
    for seg_idx, seg in enumerate(test_segments):
        ...
        # Align predictions and true sequence
        pred_seq = np.array(predictions[:len(seg) - input_len])
        true_seq = seg[input_len:input_len + len(pred_seq)]
    ...
    print("Test prediction done for all segments.")
    ...
    for t in range(input_len, beat_length):
        ...
        for seg_idx, seg in enumerate(test_segments):
            ...
            # ========= 1) GET INPUT WINDOW =========
            y_in = seg[t - input_len : t].reshape(1, -1)       # shape (1, input_len)
            ...
            with torch.no_grad():
                x_t = torch.from_numpy(y_in_pca).float().to(device)   # (1, pca_comp)
                y_pred = model(x_t).cpu().numpy().reshape(-1)        # (pred_len,)
    
            # Take only the next predicted value for online prediction
            pred_val = float(y_pred[0])   # predict next single sample (first element)
