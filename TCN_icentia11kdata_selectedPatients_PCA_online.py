...
class TemporalBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride, dilation, padding, activation='relu'):
        super().__init__()
        self.conv1 = nn.Conv1d(in_channels, out_channels, kernel_size,
                               stride=stride, padding=padding, dilation=dilation)
        self.chomp1 = Chomp1d(padding)
        self.relu1 = nn.ReLU() if activation == 'relu' else nn.Tanh()
...
class TCNRegressor(nn.Module):
    def __init__(self, input_dim, hidden_channels, output_dim, num_layers=1, kernel_size=3):
        super().__init__()
        layers = []
...
save_dir = f"results_all_selected_patients_TCN"
os.makedirs(save_dir, exist_ok=True)

results_csv = os.path.join(
    save_dir,
    f"results_summary_TCN_online_detection_PCA_all_patient_{record}_data_seed_{data_seed}_res_seed_{res_seed}_input_len_{input_len}_batch_size_{batch_size}_hidden_dim_{hidden_dim}.csv"
)
...
for pred_len in prediction_lengths:
    ...
    X_t = torch.from_numpy(X_y_all_norm.T).float().reshape(-1, pca_comp, 1).to(device)
    Y_t = torch.from_numpy(Y_all_norm.T).float().to(device)
    
    dataset = torch.utils.data.TensorDataset(X_t, Y_t)
    loader = torch.utils.data.DataLoader(dataset, batch_size=batch_size, shuffle=True)
    
    # ====== Initialize Model ======
    model = TCNRegressor(input_dim=1, hidden_channels=hidden_dim,
                         output_dim=pred_len, num_layers=num_layers).to(device)
    ...
    # ====== PREDICTION ON EACH SEGMENT (TCN) ======
    model.eval()
    all_predictions = []
    all_true = []
    ...
    for seg_idx, seg in enumerate(train_segments):
        ...
        while start + pred_len <= len(seg):
            ...
            # Reshape for TCN input: (batch=1, seq_len=input_len, input_dim=1)
            x_t = torch.from_numpy(y_in_norm.reshape(1, pca_comp, 1)).float().to(device)
    
            # Predict using trained TCN
            with torch.no_grad():
        ...
        # Align prediction length with true sequence
    ...
    print("Prediction done for all segments (TCN).")
    ...
    print("Validation prediction done for all segments (TCN).")
    ...
    print("Test prediction done for all segments (TCN).")
