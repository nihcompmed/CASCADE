...
annotation_folder = 'icentia_data'
...
# ====== Parameters ======
beat_length = 150  # samples before the beat
num_train_beats = 500
num_val_beats = 100
...
save_dir = f"results_all_selected_patients_multiPara_seeds_icentiaData"
os.makedirs(save_dir, exist_ok=True)

results_csv = os.path.join(
    save_dir,
    f"results_summary_online_detection_PCA_all_patient_multiParaset_{record}_data_seed_{data_seed}_res_seed_{res_seed}_input_len_{input_len}_reservoirs_{N}_regime_id_{regime_id}.csv"
)
...
if os.path.exists(results_csv):
    ...
    df_existing["Regime_ID"] = df_existing["Regime_ID"].astype(int)
    ...
    existing_configs = set(
        zip(
            ...
            df_existing["N"],
            df_existing["Regime_ID"]  # include regime_id here
        )
    )
else:
    existing_configs = set()
...
# ====== PATIENT FROM ARG ======
patient = record   # rename for clarity

patient_folder = os.path.join(data_folder, "p00", patient)

# get all WFDB records inside patient
records = sorted([
    f.replace(".hea", "")
...
print(f"Patient: {patient}, Records found: {len(records)}")
...
for rec_name in records:
    ...
    try:
        ...
        print(f"Skipping {rec_name}, error: {e}")
        continue

    signal = rec.p_signal[:, 0]   # Lead I

    # Extract beats
    norm_seg, _ = get_segments(signal, ann.sample, ann.symbol, beat_length, NORMAL_BEATS)
    arr_seg, _  = get_segments(signal, ann.sample, ann.symbol, beat_length, ARRHYTHMIC_BEATS)

    normal_segments.extend(norm_seg)
    arr_segments.extend(arr_seg)

print(f"Total Normal: {len(normal_segments)}, Arrhythmic: {len(arr_segments)}")
...
regime_range = parameter_regimes[regime_id]

print(f"\n====== Running Parameter Regime {regime_id} ======")

# Generate parameters for this regime
params_fixed = np.array([
    [rng_res.uniform(*r) for r in regime_range] + [rng_res.uniform(*tau_range)]
...
for pred_len in prediction_lengths:
    ...
    if (str(record), int(data_seed), int(reservoir_seed), int(input_len), int(pred_len), int(N), int(regime_id)) in existing_configs:
        print(f"Skipping existing config: Patient={record}, Data_seed={data_seed}, Reservoir_seed = {reservoir_seed}, input_len={input_len}, Pred_len={pred_len}, N={N}, Regime_ID={regime_id}")
        continue
            
    print(f"Running experiment for Patient={record}, Data_seed={data_seed}, Reservoir_seed = {reservoir_seed}, input_len={input_len}, Pred_len={pred_len}, N={N}, Regime_ID={regime_id}")

    # ====== TRAIN SINGLE DSRN ======
    S, R, params, param_means, param_stds = train_dsrn(X_y_all_norm, Y_all_norm, N, steps, params_fixed)
    print(f"--> Training done for Patient={record}, Data_seed={data_seed}, Reservoir_seed = {reservoir_seed}, input_len={input_len}, Pred_len={pred_len}, N={N}, Regime_ID={regime_id}.")
    ...
    # ====== FIRST ANOMALY DETECTION TIME (per segment) ======
    first_detection_time = np.full(num_segs, np.nan)
    ...
    for t in range(input_len, beat_length):
        ...
        for seg_idx, seg in enumerate(test_segments):
            ...
            for seg_idx in range(num_segs):
                if np.isnan(first_detection_time[seg_idx]) and current_flags[seg_idx] == 1:
                    first_detection_time[seg_idx] = t
    ...
    first_detection_time = np.array(first_detection_time)

    normal_first_times = first_detection_time[:num_test_norm]
    arr_first_times = first_detection_time[num_test_norm:num_test_norm + num_test_arr]

    avg_first_detection_normal = np.nanmean(normal_first_times)
    avg_first_detection_arr = np.nanmean(arr_first_times)
    avg_first_detection_all = np.nanmean(first_detection_time)

    std_first_detection_arr = np.nanstd(arr_first_times)
    ...
    results_dict = {
        ...
        "Avg_First_Detection_Time_All": avg_first_detection_all,
        "Avg_First_Detection_Time_Normal": avg_first_detection_normal,
        "Avg_First_Detection_Time_Arrhythmic": avg_first_detection_arr,

        "Std_First_Detection_Time_Arrhythmic": std_first_detection_arr,
    ...
    # ====== CHECK IF N EXISTS ======
    key_cols = ["Patient", "Data_seed", "Reservoir_seed", "input_len", "Pred_len", "N", "Regime_ID"]
    ...
    exists = (
        ...
        (df_existing["N"] == N) &
        (df_existing["Regime_ID"] == regime_id)
    ...
    if exists:
        print(
            ...
            f"Patient={record}, Data_seed={data_seed}, Reservoir_seed={reservoir_seed}, input_len={input_len}, Pred_len={pred_len}, N={N}, Regime_ID={regime_id}. "
            "Skipping save."
        )
    else:
